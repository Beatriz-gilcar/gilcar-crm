...
import { toggleVendaProtecaoStatus, deleteVendaProtecao } from '../actions'
...
export default async function ProtecaoPage({
  ...
  const { data: vendasData } = await supabase
    ...
    .select(
      'id, numero_sequencial, consultor_id, unidade_id, descricao, valor, data, status, profiles(nome), unidades(nome)'
    )
    .gte('data', inicio)
    .lt('data', fim)
      ...
      <div className="flex flex-1 flex-col gap-6 px-4 py-8 sm:px-10">
        <div className="mx-auto w-full max-w-4xl">
          ...
          <div className="sec-body mt-6" style={{ padding: 0 }}>
            {vendas.length === 0 ? (
              <div className="empty-state">Nenhuma proteção lançada em {mes}.</div>
            ) : (
                      ...
                      {isGerencia && (
                        <form action={deleteVendaProtecao}>
                          ...
                          <ConfirmButton
                            ...
                            confirmMessage={`Excluir a proteção #${v.numero_sequencial} definitivamente?`}
                          >
                            Excluir
                          </ConfirmButton>
                        </form>
